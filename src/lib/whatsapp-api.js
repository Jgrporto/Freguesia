...
export const normalizeWhatsappConversation = (conversation = {}) => {
  ...
  return {
    ...
    tags: manualTags,
    labels,
    label_ids: labels.map((label) => String(label?.id || '')).filter(Boolean),
    ...
    queue_status: conversation.queue_status || conversation.queueStatus || '',
    resolution_status: conversation.resolution_status || conversation.resolutionStatus || '',
    resolution_type: conversation.resolution_type || conversation.resolutionType || '',
    resolved_at: conversation.resolved_at || conversation.resolvedAt || '',
    resolved_until: conversation.resolved_until || conversation.resolvedUntil || '',
    resolved_by_id: conversation.resolved_by_id || conversation.resolvedById || '',
    resolved_by_name: conversation.resolved_by_name || conversation.resolvedByName || '',
...
export const fetchWhatsappConversations = async () => {
  const data = await requestWhatsappJson('/api/whatsapp/conversations', {
    method: 'GET',
  });

  const list = Array.isArray(data) ? data : [];
  return aggregateConversationsByCustomerPhone(list);
};
