...
export const normalizeConversationPreference = (preference = {}) => ({
  id: String(preference?.id || preference?.conversation_id || '').trim(),
  conversation_id: String(preference?.conversation_id || preference?.id || '').trim(),
...
export const fetchConversationPreferences = async () => {
  ...
  return Array.isArray(data) ? data.map(normalizeConversationPreference) : [];
};
