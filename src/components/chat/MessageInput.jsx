...
export default function MessageInput({
  ...
  const attachmentMenuItems = useMemo(
    () => [
      ...
      },
      {
        ...
        onClick: () => {
          setShowAttachmentMenu(false);
          toast.message('Compartilhamento de contato ainda não está disponível neste painel.');
        },
      },
    ],
    []
  ...
  const handleInsertEmoji = (emoji) => {
    ...
    const nextValue = `${value.slice(0, selectionStart)}${nextEmoji}${value.slice(selectionEnd)}`;
    onValueChange(nextValue);
    setShowEmojiPicker(false);
    closeQuickReplyPicker();

    requestAnimationFrame(() => {
      const cursorPosition = selectionStart + nextEmoji.length;
      textarea.focus();
        ...
        <div className="flex items-center gap-2">
          ...
          {isRecording ? (
            ...
            <>
              ...
              <Popover
                open={showEmojiPicker && canSendFreeText}
                onOpenChange={(open) => {
              ...
              >
                <PopoverTrigger asChild>
                  <Button
                    ...
                    title="Inserir emoji"
                    disabled={!canSendFreeText || Boolean(audioPreview)}
                  >
                ...
                <PopoverContent
                  align="start"
                  side="top"
                  className="w-[280px] rounded-2xl border-border bg-card p-3 shadow-xl"
                >
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-[11px] font-medium text-muted-foreground">Emojis</span>
                    <span className="text-[11px] text-muted-foreground">Clique para inserir</span>
                  </div>
                  <div className="grid grid-cols-6 gap-1.5">
                    {EMOJI_OPTIONS.map((emoji) => (
                      <button
                        ...
                        className="flex h-10 items-center justify-center rounded-xl text-xl transition-colors hover:bg-muted"
                      >
                        {emoji}
                      </button>
                    ))}
