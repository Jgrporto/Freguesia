...
import { fetchCustomerSyncState, fetchPersistedCustomers } from '@/lib/customer-sync-api';
...
const CHATBOT_RUNTIME_REFRESH_INTERVAL_MS = 15000;
...
export default function SiteNotificationBridge() {
  ...
  const { data: rawConversations = [] } = useQuery({
    ...
    refetchInterval: notificationSettings.alertNewConversations ? LABEL_REFRESH_INTERVAL_MS : false,
    staleTime: 10000,
  ...
  useEffect(() => {
    ...
    const timerId = window.setTimeout(() => {
      if (!hasStoredBrowserSyncConfig()) {
        return;
      }

      try {
        const config = readStoredBrowserSyncConfig();
